//
//  MagicCrayonMetalProcessor.swift
//  IosTest1
//
//  移植自 MagicCamera Android：`crayon.glsl` + `MagicCrayonFilter`（strength 默认 0.5）
//
...
/// BGRA 同尺寸 in/out；在 `Shaders.metal` 的 `fragment_magic_crayon` 上渲染全屏四边形
final class MagicCrayonMetalProcessor {
    ...
    init?() {
        guard let dev = MTLCreateSystemDefaultDevice(),
              ...
              let ffn = lib.makeFunction(name: "fragment_magic_crayon")
        else { return nil }
    ...
    func applyIfPossible(to image: CIImage, context: CIContext) -> CIImage {
        ...
        context.render(image, to: inputPB, bounds: bounds, colorSpace: cs)
